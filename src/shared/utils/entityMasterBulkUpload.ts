...
/** Download CSV template for entity master bulk upload. */
export function downloadEntityMasterUploadTemplate(): void {
  const templateLine = [
...
export function importEntityMastersFromCsv(
  text: string,
  ...
  dataRows.forEach((cells, index) => {
    ...
    if (nameKey && existingNames.has(nameKey)) {
      skipped.push({ row: rowNumber, reason: `Entity "${formData.entityName}" already exists.` })
      return
    }
