...
function resolveVesselType(raw: string): VesselType | undefined {
  ...
  const byValue = VESSEL_TYPE_OPTIONS.find((option) => option.value === normalized)
  if (byValue) return byValue.value

  const byLabel = VESSEL_TYPE_OPTIONS.find((option) => option.label.toLowerCase() === normalized)
  if (byLabel) return byLabel.value

  const byPartialLabel = VESSEL_TYPE_OPTIONS.find((option) =>
...
/** Download CSV template for vessel master bulk upload. */
export function downloadVesselMasterUploadTemplate(): void {
  const exampleType = VESSEL_TYPE_OPTIONS.find((option) => option.value === 'bulk_carrier')?.label ?? 'Bulk carrier'
  const templateLine = [
    '',
    'Example Vessel',
    '1234567',
...
export function importVesselMastersFromCsv(
  text: string,
  ...
  dataRows.forEach((cells, index) => {
    ...
    const getCell = (cellIndex: number) => (cells[cellIndex] ?? '').trim()
