...
export function VendorServiceMappingSection({ data, onChange }: VendorServiceMappingSectionProps) {
    ...
    <Stack spacing={2}>
      ...
      {data.serviceMappings.length === 0 ? (
        <Box sx={{ py: 3, textAlign: 'center' }}>
          <Typography variant="body2" color="text.secondary">
            No services mapped yet. Add a service to define rates.
          </Typography>
        </Box>
      ) : (
                  ...
                  <TableRow key={mapping.id}>
                    ...
                    <TableCell align="center">
                      <Stack direction="row" spacing={0.5} justifyContent="center">
                        <IconButton icon={<PencilLine size={14} />} tooltip="Edit mapping" onClick={() => openEdit(mapping)} />
                        <IconButton icon={<Trash2 size={14} />} tooltip="Remove mapping" onClick={() => handleRemove(mapping.id)} />
                      </Stack>
                    </TableCell>
                  </TableRow>
                )
