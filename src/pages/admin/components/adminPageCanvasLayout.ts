...
/** Soft accent hues for admin page canvas (module-scoped, not global brand tokens). */
const ADMIN_PAGE_CANVAS_PURPLE_LIGHT = '#6366F1'
const ADMIN_PAGE_CANVAS_PURPLE_DARK = '#A78BFA'
const ADMIN_PAGE_CANVAS_YELLOW_LIGHT = '#FBBF24'
const ADMIN_PAGE_CANVAS_YELLOW_DARK = '#FCD34D'

function getAdminPageCanvasPurple(theme: Theme): string {
  return theme.palette.mode === 'light'
    ? ADMIN_PAGE_CANVAS_PURPLE_LIGHT
    : ADMIN_PAGE_CANVAS_PURPLE_DARK
}

function getAdminPageCanvasYellow(theme: Theme): string {
  return theme.palette.mode === 'light'
    ? ADMIN_PAGE_CANVAS_YELLOW_LIGHT
    : ADMIN_PAGE_CANVAS_YELLOW_DARK
}

function getAdminPageCanvasFlowOpacities(theme: Theme) {
...
function buildAdminPageCanvasFlowBackground(
  theme: Theme,
  colors: PublicBrandColors,
  scale: 'page' | 'panel',
): string {
  const purple = getAdminPageCanvasPurple(theme)
  const yellow = getAdminPageCanvasYellow(theme)
  const navy = theme.palette.primary.main
  const green = colors.green
  const o = getAdminPageCanvasFlowOpacities(theme)
...
/** Bleed into AppShell `<main>` padding so canvas background fills the content area. */
export const ADMIN_PAGE_CANVAS_MAIN_BLEED_SX = {
...
export function getAdminPageCanvasBackground(theme: Theme, colors: PublicBrandColors): string {
  return buildAdminPageCanvasFlowBackground(theme, colors, 'page')
}
...
export function getAdminPageCanvasShellSx(
  ...
  return {
    ...ADMIN_PAGE_CANVAS_MAIN_BLEED_SX,
    bgcolor: theme.palette.background.default,
    backgroundImage: getAdminPageCanvasBackground(theme, colors),
...
export function getAdminPageCanvasPanelBackgroundSx(
  ...
  return {
    ...
    backgroundImage: buildAdminPageCanvasFlowBackground(theme, colors, 'panel'),
    backgroundRepeat: 'no-repeat',
    backgroundSize: 'cover',
  }
}
