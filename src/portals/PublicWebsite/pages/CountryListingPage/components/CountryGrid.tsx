...
import { publicColors, publicTypography } from '../../../theme/publicSiteTokens'
  ...
  if (filtered.length === 0) {
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 12, gap: 2 }}>
        <Globe size={48} color={publicColors.textMuted} />
        <Typography sx={{ fontWeight: 700, fontSize: '20px', color: publicColors.navy }}>
          No destinations found
        </Typography>
        <Typography sx={{ color: publicColors.textSecondary }}>Try adjusting your search or filters</Typography>
      </Box>
    )
  }
    ...
    <>
      ...
      <Grid container spacing={4}>
        {filtered.map(country => (
          <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={country.id}>
